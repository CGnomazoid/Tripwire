...
- `src/supervisor/audit_log.py` - every tool accepts an optional `reason` (the calling agent's own stated justification for the action). It is **never** passed into `Judge.ask()` or the model's prompt - the risk score stays a pure function of `state` alone - it's only logged (`data/audit_log.jsonl`, gitignored) and echoed back in the response, surfaced in `should_block`'s `message` field specifically when the decision is a block. The point: give a human reviewing a denial some context on what the agent thought it was doing, without opening a channel for a fluent-sounding justification to talk the judge into a lower score. `demo/agent_demo.py` deliberately gives its two blocked calls plausible-sounding reasons ("cleaning up temp files before the next run" for a full recursive delete) to demonstrate the score doesn't move.
- `tests/` - fast pure-math unit tests for the calibration layer, pydantic validators, and the audit log; integration tests against the real model (`test_judge.py`); end-to-end tests of the MCP server over the real stdio protocol including the reason/audit-log behavior (`test_mcp_server.py`) - all model-touching tests are marked `slow` and double as safety checks (byte-identical repo hash before/after judging deliberately destructive inputs).
